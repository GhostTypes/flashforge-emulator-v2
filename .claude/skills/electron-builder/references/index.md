...
# electron-builder [npm version](https://www.npmjs.com/package/electron-builder) [downloads per month](https://yarn.pm/electron-builder) [donate](https://www.electron.build/donate)
...
|  |
| --- |
| [WorkFlowy Notes, Tasks, Projects. All in a Single Place.](https://workflowy.com) |
| [Tidepool Your gateway to understanding your diabetes data](https://tidepool.org) | [Keygen An open, source-available software licensing and distribution API](https://keygen.sh/?via=electron-builder) |
| [ToDesktop ToDesktop: An all-in-one platform for building and releasing Electron apps](https://www.todesktop.com/electron?utm_source=electron-builder) | [Dashcam Dashcam: Capture the steps to reproduce any bug with video crash reports for Electron.](https://www.dashcam.io/?ref=electron_builder) |
...
1. Specify the standard fields in the application `package.json` — [name](https://electron.build/configuration#metadata), `description`, `version` and [author](https://docs.npmjs.com/files/package.json#people-fields-author-contributors).
2. Specify the [build](https://www.electron.build/configuration#build) configuration in the `package.json` as follows:
...
5. If you have native addons of your own that are part of the application (not as a dependency), set [nodeGypRebuild](https://www.electron.build/configuration#nodegyprebuild) to `true`.

Please note that everything is packaged into an asar archive [by default](https://electron.build/configuration#asar).
